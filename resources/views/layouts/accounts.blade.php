...
<body class="min-h-screen bg-zinc-950 text-white antialiased">
    ...
    <header class="sticky top-0 z-40 border-b border-white/6 bg-zinc-950/95 backdrop-blur-md">
        <div class="mx-auto flex h-13 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">

            <div class="flex items-center gap-4">
                <img src="{{ Vite::asset('resources/assets/images/logo.svg') }}" alt="{{ config('app.name') }}"
                    class="h-6 w-auto" />
                <div class="h-4 w-px bg-white/10"></div>
                <span class="text-[11px] font-medium tracking-[0.12em] uppercase text-white/30">Account</span>
            </div>

            <div class="flex items-center gap-3">
                @auth
                    <a href="{{ auth()->user()->redirect() }}"
                        class="hidden items-center gap-2 border border-white/10 px-3 py-1.5 text-xs text-white/40 transition-colors hover:bg-white/5 hover:text-white/70 sm:flex">
    ...
    {{-- Mobile nav --}}
    <div class="border-b border-white/6 bg-zinc-950 lg:hidden">
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="flex gap-10">
            ...
            <aside class="hidden w-52 shrink-0 lg:flex lg:flex-col">
                <nav class="flex-1 space-y-px">
                    ...
                    <a href="{{ route('account.index') }}" wire:navigate
                        class="group flex items-center gap-3 border-l-2 px-4 py-2.5 text-sm transition-colors
                            {{ $profileActive ? 'border-blue-400 bg-blue-500/7 text-white' : 'border-transparent text-white/40 hover:border-white/15 hover:bg-white/3 hover:text-white/75' }}">
                        <flux:icon.user class="size-4 shrink-0 {{ $profileActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Profile
                    </a>
                    ...
                    @if (Route::has('account.security'))
                        ...
                        <a href="{{ route('account.security') }}" wire:navigate
                            class="group flex items-center gap-3 border-l-2 px-4 py-2.5 text-sm transition-colors
                                {{ $secActive ? 'border-blue-400 bg-blue-500/7 text-white' : 'border-transparent text-white/40 hover:border-white/15 hover:bg-white/3 hover:text-white/75' }}">
                            <flux:icon.lock-closed class="size-4 shrink-0 {{ $secActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                            Security
                        </a>
                    @else
                        <span class="flex cursor-not-allowed items-center gap-3 border-l-2 border-transparent px-4 py-2.5 text-sm text-white/20">
                            <flux:icon.lock-closed class="size-4 shrink-0 text-white/15" />
                            Security
                    ...
                    <a href="{{ route('account.two-factor-setup') }}" wire:navigate
                        class="group flex items-center gap-3 border-l-2 px-4 py-2.5 text-sm transition-colors
                            {{ $tfaActive ? 'border-blue-400 bg-blue-500/7 text-white' : 'border-transparent text-white/40 hover:border-white/15 hover:bg-white/3 hover:text-white/75' }}">
                        <flux:icon.shield-check class="size-4 shrink-0 {{ $tfaActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Two-Factor
                    </a>
                    ...
                    @if (Route::has('account.export'))
                        ...
                        <a href="{{ route('account.export') }}" wire:navigate
                            class="group flex items-center gap-3 border-l-2 px-4 py-2.5 text-sm transition-colors
                                {{ $exportActive ? 'border-blue-400 bg-blue-500/7 text-white' : 'border-transparent text-white/40 hover:border-white/15 hover:bg-white/3 hover:text-white/75' }}">
                            <flux:icon.arrow-down-tray class="size-4 shrink-0 {{ $exportActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                            Export Data
                        </a>
                    @endif
                    ...
                    @if (Route::has('account.settings'))
                        ...
                        <a href="{{ route('account.settings') }}" wire:navigate
                            class="group flex items-center gap-3 border-l-2 px-4 py-2.5 text-sm transition-colors
                                {{ $settActive ? 'border-blue-400 bg-blue-500/7 text-white' : 'border-transparent text-white/40 hover:border-white/15 hover:bg-white/3 hover:text-white/75' }}">
                            <flux:icon.cog-6-tooth class="size-4 shrink-0 {{ $settActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                            Settings
                        </a>
                    @else
                        <span class="flex cursor-not-allowed items-center gap-3 border-l-2 border-transparent px-4 py-2.5 text-sm text-white/20">
                            <flux:icon.cog-6-tooth class="size-4 shrink-0 text-white/15" />
                            Settings
                            <span class="ml-auto text-[10px] text-white/20">Soon</span>
                        </span>
                    @endif
                ...
                <div class="mt-auto border-t border-white/6 pt-2">
