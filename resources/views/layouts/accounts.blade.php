            ...
            <aside class="hidden w-52 shrink-0 lg:flex lg:flex-col top-21.25 h-[calc(100vh-95px)] sticky">
                <nav class="min-h-0 flex-1 overflow-y-auto space-y-px scrollbar-none [&::-webkit-scrollbar]:hidden">
                    ...
                    <a href="{{ route('account.index') }}" wire:navigate
                        ...
                        <flux:icon.user
                            class="size-4 shrink-0 {{ $profileActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Profile
                    </a>
                    ...
                    <a href="{{ route('account.security') }}" wire:navigate
                        ...
                        <flux:icon.lock-closed
                            class="size-4 shrink-0 {{ $secActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Security
                    </a>
                    @else
                    <span
                    ...
                    <a href="{{ route('account.two-factor-setup') }}" wire:navigate
                        ...
                        <flux:icon.shield-check
                            class="size-4 shrink-0 {{ $tfaActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Two-Factor
                    </a>
                    ...
                    <a href="{{ route('account.export') }}" wire:navigate
                        ...
                        <flux:icon.arrow-down-tray
                            class="size-4 shrink-0 {{ $exportActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Export Data
                    </a>
                    @endif
                    ...
                    <a href="{{ route('account.settings') }}" wire:navigate
                        ...
                        <flux:icon.cog-6-tooth
                            class="size-4 shrink-0 {{ $settActive ? 'text-blue-400' : 'text-white/25 group-hover:text-white/50' }}" />
                        Settings
                    </a>
                    @else
                    <span
