...
<div class="flex min-h-screen flex-col bg-white dark:bg-zinc-950">
    ...
    <div class="flex flex-1 items-center justify-center px-4 py-12">
        <div class="w-full max-w-sm">
            ...
            @if (session('status'))
            <div
                class="mb-4 border border-emerald-500/20 bg-emerald-500/6 px-4 py-3 text-center text-sm text-emerald-400">
                {{ session('status') }}
            </div>
            @endif
            ...
            <flux:card class="p-5! space-y-5">
                ...
                <div class="flex border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/3">
                    <button type="button" wire:click="switchTab('phone')"
                        @class([ 'flex-1 py-2 text-sm font-medium transition-all'
                        , 'bg-zinc-100 dark:bg-white/8 text-zinc-900 dark:text-white'=> $activeTab === 'phone',
                    ...
                    <button type="button" wire:click="switchTab('email')"
                        @class([ 'flex-1 py-2 text-sm font-medium transition-all'
                        , 'bg-zinc-100 dark:bg-white/8 text-zinc-900 dark:text-white'=> $activeTab === 'email',
                        'text-zinc-500 dark:text-white/40 hover:text-zinc-600 dark:hover:text-white/65' => $activeTab
                        !== 'email',
                        ])>
                        {{ __('Email') }}
                ...
                <div
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-zinc-800 dark:text-white/85">
                            @if ($activeTab === 'phone')
                            {{ $country_code }}
                            <span class="text-zinc-500 dark:text-white/40">••••••</span>{{ substr(preg_replace('/\D/',
                            '', $phone), -3) }}
                            @else
                            {{ $maskedEmail }}
                            @endif
