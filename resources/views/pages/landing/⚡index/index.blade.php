@php $title = __('One Codebase, Every Domain'); @endphp
...
<div class="mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 py-16 sm:px-8">
    ...
    <div class="mx-auto max-w-2xl text-center">
        ...
        <h1 class="mt-6 text-4xl leading-tight font-semibold tracking-tight text-zinc-900 dark:text-white sm:text-5xl">
            {{ __('One codebase.') }}<br>
            <span class="text-blue-600 dark:text-blue-400">{{ __('Every') }}</span> {{ __('domain.') }}
        </h1>
        ...
        <div class="mt-8 flex items-center justify-center gap-3">
            @if (Route::has('login'))
            <flux:button variant="primary" :href="route('login')">{{ __('Sign in') }}</flux:button>
            @endif
    ...
    <div class="mx-auto mt-16 grid w-full max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <flux:card as="a" href="https://github.com/lianmaymesi/laravel-multidomain-starter/blob/main/config/multidomain.php"
            ...
            <flux:heading size="sm">{{ __('Domain-based routing') }}</flux:heading>
            <flux:text class="text-sm text-zinc-500 dark:text-white/50">{{ __('Each portal is registered in') }} <code class="text-zinc-700 dark:text-white/70">config/multidomain.php</code> {{ __('and routed with') }} <code class="text-zinc-700 dark:text-white/70">Route::domain()</code> — {{ __('no shared middleware, no namespace bleed.') }}</flux:text>
        ...
        <flux:card as="a" href="https://github.com/lianmaymesi/laravel-multidomain-starter/blob/main/app/Console/Commands/MakeSubdomainCommand.php"
            ...
            <flux:heading size="sm">{{ __('Scaffold a portal in seconds') }}</flux:heading>
            <flux:text class="text-sm text-zinc-500 dark:text-white/50"><code class="text-zinc-700 dark:text-white/70">php artisan make:subdomain</code> {{ __('generates the route file, layout, Vite assets and a starter dashboard for a brand-new portal instantly.') }}</flux:text>
            <flux:text class="text-sm text-blue-600 dark:text-blue-400">{{ __('View command →') }}</flux:text>
        </flux:card>

        <flux:card as="a" href="https://livewire.laravel.com" target="_blank" rel="noopener" class="space-y-2">
            <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-500/15">
                <flux:icon.bolt class="size-4.5 text-blue-600 dark:text-blue-400" />
            </div>
            <flux:heading size="sm">{{ __('Livewire 4, everywhere') }}</flux:heading>
            <flux:text class="text-sm text-zinc-500 dark:text-white/50">{{ __('Pages are single-file Livewire components — reactive, server-driven UI in every portal, no separate API layer required.') }}</flux:text>
            <flux:text class="text-sm text-blue-600 dark:text-blue-400">{{ __('Livewire docs →') }}</flux:text>
        </flux:card>

        <flux:card as="a" href="https://fluxui.dev" target="_blank" rel="noopener" class="space-y-2">
            <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-500/15">
                <flux:icon.swatch class="size-4.5 text-blue-600 dark:text-blue-400" />
            </div>
            <flux:heading size="sm">{{ __('One design system') }}</flux:heading>
            <flux:text class="text-sm text-zinc-500 dark:text-white/50">{{ __('Every portal shares the same Flux UI components and theme tokens, so new subdomains look consistent from the first commit.') }}</flux:text>
            <flux:text class="text-sm text-blue-600 dark:text-blue-400">{{ __('Flux UI docs →') }}</flux:text>
        </flux:card>
