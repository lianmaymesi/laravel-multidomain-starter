...
<div class="space-y-10">
    ...
    <section class="space-y-5">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                <flux:heading size="lg" class="text-zinc-900! dark:text-white!">Export your data</flux:heading>
                <flux:text class="text-zinc-500! dark:text-white/40! text-sm!">Download a copy of your account data. Each export is packaged as a ZIP with CSV files.</flux:text>
            </div>
        ...
        <div class="flex items-center gap-3 rounded-2xl border border-blue-500/20 bg-blue-50 dark:bg-blue-500/10 px-5 py-4">
            ...
            <div>
                <p class="text-sm font-medium text-blue-700 dark:text-blue-300">Export queued</p>
                <p class="text-xs text-blue-600 dark:text-blue-400/70 mt-0.5">Generating your export. You'll receive an email with a download link once ready.</p>
            </div>
        </div>
        @endif
        ...
        <div class="flex flex-col items-center justify-center rounded-[1.75rem] border border-zinc-200 dark:border-white/[0.07] bg-zinc-50 dark:bg-white/3 px-6 py-16 text-center">
            ...
            <p class="text-sm font-medium text-zinc-500 dark:text-white/50">No exports yet</p>
            <p class="text-xs text-zinc-400 dark:text-white/30 mt-1">Request an export above. It'll appear here once queued.</p>
        </div>
        @else
            ...
            <div class="{{ $isExpired ? 'opacity-50' : '' }}">
                ...
                <div class="grid grid-cols-[auto_1fr_auto] sm:grid-cols-[auto_1fr_1fr_1fr_auto] items-center gap-4 px-6 py-4">
                    ...
                    <div class="hidden sm:block">
                        @if ($isProcessing)
                        <p class="text-xs text-zinc-400 dark:text-white/30">Preparing…</p>
                        @elseif ($isReady)
                        ...
                        @else
                        <span class="rounded-full border border-zinc-200 dark:border-white/[0.08] bg-zinc-100 dark:bg-white/5 px-2 py-0.5 text-[10px] font-medium text-zinc-400 dark:text-white/25">Expired</span>
                        @endif
                    </div>
                    ...
                    <div class="hidden sm:block">
                        ...
                        @else
                        <p class="text-xs text-zinc-400 dark:text-white/25">Never downloaded</p>
                        @endif
                    </div>
                ...
                <div class="border-t border-zinc-200 dark:border-white/5 mx-6 mb-4 pt-4 space-y-3">
                    <p class="text-xs text-zinc-500 dark:text-white/50">Enter your password to download. You can download up to 3 times per 24 hours.</p>
        ...
        <div class="flex items-start gap-3 rounded-2xl border border-zinc-200 dark:border-white/[0.05] bg-zinc-50 dark:bg-white/[0.02] px-5 py-4">
            ...
            <p class="text-xs text-zinc-400 dark:text-white/35 leading-relaxed">
                Export files are available for <span class="text-zinc-500 dark:text-white/50">{{ \App\Models\AccountDataExport::EXPORT_TTL_DAYS }} days</span> after generation, then permanently deleted. Downloads are password-protected and limited to <span class="text-zinc-500 dark:text-white/50">{{ \App\Models\AccountDataExport::MAX_DOWNLOADS_PER_DAY }} per 24 hours</span>.
            </p>
        </div>
